# [full stack nginx NodeBB for everyone with docker compose](https://github.com/damalis/full-stack-proxy-nginx-nodebb-for-everyone-with-docker-compose)

If You want to install NodeBB at short time;

#### Full stack Proxy Nginx NodeBB:
[![NodeBB](https://img.shields.io/badge/Forum-NodeBB-ff6600?style=flat&logo=nodebb&logoColor=white)](https://nodebb.org/)
...
- [Compass](https://hub.docker.com/r/haohanyang/compass-web)
...
```
git clone https://github.com/damalis/full-stack-proxy-nginx-nodebb-for-everyone-with-docker-compose.git
```

Open a terminal and `cd` to the folder in which `docker-compose.yml` is saved and run:

```
cd full-stack-proxy-nginx-nodebb-for-everyone-with-docker-compose
chmod +x install.sh
...
|```DATABASE_USERNAME```|```DATABASE_USER_PASSWORD```|```DATABASE_NAME```|```DATABASE_ROOT_PASSWORD```|
|```CW_BASIC_AUTH_USERNAME```|```CW_BASIC_AUTH_PASSWORD```|```SSL_SNIPPET```|
...
Mongodb settings; |Host: database; Port: 27017; Database: `DATABASE_NAME`; User: `DATABASE_USERNAME`; Password: `DATABASE_ROOT_PASSWORD`| in the `.env` file.
