...
echo "======================================================================="
echo "|                                                                     |"
echo "|   full-stack-proxy-nginx-nodebb-for-everyone-with-docker-compose    |"
echo "|                         by Erdal ALTIN                              |"
echo "|                                                                     |"
echo "======================================================================="
sleep 2
...
then
	...
	sudo -- sh -c -e "grep -qxF '127.0.1.1  compass.'$domain_name /etc/hosts || echo '127.0.1.1  compass.'$domain_name >> /etc/hosts"
...
# set your subdomain name
subdomain=""
regex="^[a-z0-9-]+(\.[a-z0-9-]+)*$"
read -p 'Enter Subdomain (default : forum): ' subdomain
: ${subdomain:=forum}
if [ "$which_h" == "localhost" ]
then
...
read -p 'Enter Database Password(at least 6 characters): ' db_user_password
while [[ ! $db_user_password =~ $password_regex ]]
do
...
password_regex="^[a-zA-Z0-9\._-]{6,}$"
read -p 'Enter Database Root Password(at least 6 characters): ' database_root_password
while [[ ! $database_root_password =~ $password_regex ]]
do
	echo "Try again (can only contain numerals 0-9, basic Latin letters, both lowercase and uppercase, dot, underscore and minus sign)"
	read -p 'Enter Database Root Password(at least 6 characters): ' database_root_password
	sleep 1
...
compass_username=""
read -p 'Enter Compass Username(at least 6 characters): ' compass_username
while [[ ! $compass_username =~ $db_regex ]]
do
	echo "Try again (can only contain numerals 0-9, basic Latin letters, both lowercase and uppercase, dollar sign and underscore)"
	read -p 'Enter Compass Username(at least 6 characters): ' compass_username
	sleep 1
...
compass_password=""
read -p 'Enter Compass Password(at least 6 characters): ' compass_password
while [[ ! $compass_password =~ $password_regex ]]
do
	echo "Try again (can only contain numerals 0-9, basic Latin letters, both lowercase and uppercase, dot, underscore and minus sign)"
	read -p 'Enter Compass Password(at least 6 characters): ' compass_password
	sleep 1
...
sed -i 's/db_user_password/'$db_user_password'/' .env
sed -i 's/database_root_password/'$database_root_password'/' .env
sed -i 's/db_name/'$db_name'/' .env
sed -i 's/compass_username/'$compass_username'/' .env
sed -i 's/compass_password/'$compass_password'/' .env
...
if [ -x "$(command -v docker)" ] && [ "$(docker compose version)" ]; then
	...
	# installing NodeBB and the other services
	docker compose up -d & export pid=$!
	echo "NodeBB and the other services installing proceeding..."
	echo
	wait $pid
	if [ $? -eq 0 ]
	then
		...
		echo
		echo "Error! could not installed NodeBB and the other services with docker compose" >&2
		echo
		exit 1
	fi
else
